...
type Policy struct {
	Operations   PolicyOperation  `json:"operations,omitempty"`    //基本操作权限，uint8，前4位分别表示对数据对象属性的CRUD （创建、读取、更新、删除）权限，后4位分别表示对数据内容的CRUD权限
	AlgorithmDOI string           `json:"algorithm_doi,omitempty"` //处理算法DOI，string，符合数据对象标识规范
	Weight       float64          `json:"weight,omitempty"`        //权重，uint8，取值表示百分比
...
func NewPolicyOperation(num uint8) *PolicyOperation {
	p := PolicyOperation(num)
	return &p
}
func (p *PolicyOperation) ToUInt8() uint8 { return uint8(*p) }
func (p *PolicyOperation) ToBinaryString() string {
	return biu.ToBinaryString(p.ToUInt8())
}
